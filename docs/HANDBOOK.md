...
| תשלומי מטפלים לקליניקה | **ידני** — הקליניקה רושמת | מ-24.9.2026 הסליקה מחוץ למוצר; ר' "מצב ידני" בפרק 8 |
...
אין משתני סליקה בין מטפל/ת לקליניקה — התשלום הזה ידני (פרק 8). קוד ה-
WooCommerce (`lib/woo`, `clinic_payment_settings`) נשאר במאגר רדום, בלי ממשק.
...
מחושבים, לא מיובאים: `lib/holidays.ts` הוא לוח עברי ללא תלויות עם
בדיקות מול תאריכים שפורסמו. לקליניקה שלושה מתגים על `clinics`
(`block_holidays` דלוק כברירת מחדל, `block_holiday_eves`,
`block_chol_hamoed`). יום סגור הופך ל-`room_blocks` על כל חדר פעיל, עם
`reason = 'holiday:<key>'`, דרך ה-RPC `materialize_holiday_blocks`
(service role בלבד). זה קורה מיד בשמירת ההגדרות ובכל לילה בתוך
`materialize-sessions`, 400 יום קדימה. כל מה שכבר מכבד חסימות (הלוח,
ה-board, ה-RPC של הזמנה) מכבד גם חגים בלי לדעת מהם. הזמנות שכבר יושבות
על חג נשארות; חסימות ידניות לא נוגעים בהן. ה-board מציג שם חג במקום התג.

**מיגרציה:** `20260921000001_israeli_holidays.sql` חייבת לרוץ על
Supabase לפני שהדיפלוי הזה מועיל; בלעדיה מסך ההגדרות לא יראה את
הכרטיס והריצה הלילית תרשום שגיאה ותמשיך.
...
זה החיוב ש**אנחנו** גובים מקליניקה על השימוש במערכת — נפרד לגמרי מהתשלום
של המטפלים לקליניקה, שהוא ידני (ר' "מצב ידני" בסוף הפרק).
...
### מצב ידני — תשלום המטפל/ת לקליניקה (מ-24.9.2026)

Cleana היא מערכת ניהול, לא מערכת גבייה. המטפל/ת משלם/ת לקליניקה ישירות
(מזומן, ביט, העברה, אשראי של הקליניקה), והקליניקה רושמת:

| מה נקנה | איפה רושמים | מה קורה |
| --- | --- | --- |
| כרטיסייה | מטפלים ← כרטיס המטפל/ת ← הנפקת כרטיסייה | `admin_issue_punch_card` — שעות נכנסות, שורה ב-`payments` |
| ססיה, חודש ראשון | ססיות ← "רישום תשלום" | `admin_record_session_payment` → הססיה פעילה, משבצות ננעלות |
| ססיה, חודש נוסף | ססיות ← "רישום תשלום לחודש הבא" | אותה פונקציה → `next_billing_date` זז בחודש |

הקליניקה כותבת ב**הגדרות ← איך המטפלים/ות משלמים לכם** הוראות תשלום
(`app_settings.payment_instructions`), והן מוצגות למטפלים/ות בעמוד
הכרטיסיות, בעמוד הססיות ובמייל אישור הססיה.

חיבור סליקה אמיתי יחזור כשיהיו לקוחות שמבקשים אותו — ר' PROGRESS.md.
...
`poll-woo-orders` הוסר מהלוח ב-24.9.2026 יחד עם הסליקה דרך WooCommerce;
ה-route נשאר בקוד, רדום.
...
**webhook של WooCommerce** (`/api/woo/webhook/[clinicId]`) — רדום מאז המצב
הידני (אין ממשק שמגדיר לו סוד, ובלי סוד הוא מחזיר 500). כשהוא פעיל, חתימת
HMAC מאומתת ב-`lib/woo/verify.ts` לפני כל עיבוד.
...
**מטפל/ת שילם/ה ולא רואה שעות.** במצב הידני שום דבר לא נכנס לבד: הקליניקה
צריכה לרשום את התשלום — כרטיסייה מהכרטיס של המטפל/ת, ססיה מרשימת הססיות.
כל רישום מופיע ב-/admin/payments וב-audit_log.
