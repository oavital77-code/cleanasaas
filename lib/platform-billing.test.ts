...
describe("platformPlanPriceIls", () => {
  it("defaults to the decided price", () => {
    expect(platformPlanPriceIls({})).toBe(179);
    expect(PLATFORM_PLAN_DEFAULT_PRICE_ILS).toBe(179);
  });
  it("lets the environment override it, and ignores garbage", () => {
    ...
    expect(platformPlanPriceIls({ PLATFORM_PLAN_PRICE_ILS: "abc" })).toBe(179);
    expect(platformPlanPriceIls({ PLATFORM_PLAN_PRICE_ILS: "-5" })).toBe(179);
  });
});
