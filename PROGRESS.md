...
**החלטת תמחור (8.9.2026, עודכנה 24.9.2026):** מסלול בתשלום אחד — **₪179 לחודש כולל מע״מ**
(היה ₪209; ירד כשהסליקה בין מטפל/ת לקליניקה יצאה מהמוצר — ר' "מצב ידני" למטה) —
...
אידמפוטנטיות לפי `transaction_uid`. המחיר: `PLATFORM_PLAN_PRICE_ILS` (ברירת מחדל 179).
תשלום בסכום **גבוה** מהמחיר הנוכחי מפעיל את המנוי (דף שנפתח לפני ירידת מחיר);
רק תשלום חסר נדחה (מיגרציה `20260924000001`).
...
**בוצע מאז**: superadmin ראשון נוצר (חשבון הבעלים) — קדם לו תיקון
...
- **superadmin ראשון — בוצע**: חשבון הבעלים (ר' סעיף 8 למעלה,
...
## ✅ השקה: מוכנות ל-100 משתמשים (10.9.2026)

ניתוח מלא בשיחה מ-10.9. מה תוקן בקוד: אזור Vercel `fra1` (היה `syd1` מול
Supabase ב-`eu-central-1`), 27 אינדקסים על מפתחות זרים (`20260910000001`,
הוחל על ה-DB החי), `maxDuration = 60` על ה-crons; ב-Cleana+: תזכורות באצווה
של 150 עם 3 במקביל (הלולאה הסדרתית הייתה נחתכת בטיימאאוט), pool של 3
חיבורים ל-Prisma, הגבלת קצב על נקודת הזמינות הציבורית.

**דשבורד בעלים** ב-`/superadmin/owner` (סופר-אדמין בלבד): חשבונות, מצב מנוי
והכנסות משני המוצרים (Cleana+ דרך `/api/owner/stats` שלו עם `OWNER_STATS_SECRET`),
ו**מדרגות רכישה** (`lib/owner/thresholds.ts`): באיזה מספר משתמשים פעילים
קונים איזה שירות — Supabase Pro במשלם הראשון (אין גיבוי בחינמי), Resend Pro
ב-15, Vercel Pro ב-20, מחשוב Small ב-50, PITR ב-100, וכו'. המחירים משוערים;
לשנות את הקובץ = הדשבורד מתעדכן.

**מה שנשאר ביד המשתמש:** התוכניות עצמן (Supabase/Resend/Vercel), לוודא
ש-`DATABASE_URL` של Cleana+ הוא ה-pooler, ולהזין `OWNER_STATS_SECRET` בשני
הפרויקטים.
...
- **webhook `user.deleted` — בוצע** (`app/api/webhooks/clerk/route.ts`,
  ...
  נעשתה בטעות מול הפרופיל האמיתי של הבעלים במקום
...
- **2026-09-24 — מצב ידני + ₪179.** הסליקה בין מטפל/ת לקליניקה יצאה מהמוצר: המטפל/ת משלם/ת לקליניקה ישירות והקליניקה רושמת (כרטיסייה — `admin_issue_punch_card`; ססיה — `admin_record_session_payment`, חדש). ממשק ה-WooCommerce הוסר (הגדרות, כפתורי "לתשלום בחנות", בדיקת רכישה ממתינה, עמודי החזרה מהחנות, ה-cron היומי); הקוד ב-`lib/woo` וה-webhook נשארו רדומים. במקומם: "הוראות תשלום" שהקליניקה כותבת ומוצגות למטפלים/ות ובמייל האישור — שנשלח עכשיו תמיד, ולא רק כשהוגדרה חנות. מחיר הפלטפורמה ירד ל-₪179; תשלום בסכום גבוה מהמחיר מפעיל (דף שנפתח לפני הירידה). **דורש הרצת המיגרציות `20260924000001` ו-`20260924000002`.** חיבור סליקה יחזור כשקליניקות יבקשו — ההחלטה (שותף סליקה אחד מול "תביאו את הסולק שלכם") פתוחה.
